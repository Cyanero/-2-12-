...
public:
    Pipeline()
    {
        m_scale = Vector3f(0.5f, 0.5f, 0.5f);  // Можно поменять масштаб треугольника 
...
void Pipeline::InitTranslationTransform(Matrix4f& m) const       // Матрица, которая отвечает за место положение камеры 
{
...
void Pipeline::InitPerspectiveProj(Matrix4f& m) const    // Матрица, которая отвечает за позицию камеры 
{
...
static void RenderSceneCB()
{
    ...
    p.WorldPos(0.0f, 0.0f, 5.0f); // Позиция треугольника 
    p.SetPerspectiveProj(30.0f, WINDOW_WIDTH, WINDOW_HEIGHT, 1.0f, 100.0f);  // Положение камеры 
